...
const SignUp = ({ refetch ,history}) => {
  ...
  const validate = () => {
    if (!fullName || !email || !username || !password) {
      if (!fullName) setError({ ...error, fullName: 'all field are required' })
      else if (!email) setError({ ...error, email: 'all field are required' })
      else if (!username)
        setError({ ...error, username: 'all field are required' })
      else if (!password)
