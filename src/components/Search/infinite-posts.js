          ...
          {hits.map(hit => (
            <a href={`/${hit.author && hit.author.username}`}>
              <div class="card border list-group-item list-group-item-action">
                <div class="card-body">
                  <h5 class="card-title text-capitalize">{hit.author && hit.author.firstName} {hit.author && hit.author.lastName}</h5>
                  <h6 class="card-subtitle mb-2 text-muted">@{hit.author && hit.author.username}</h6>
                  <p class="card-text text-dark">{hit.content}</p>
                </div>
              </div>
            </a>
          ))}
